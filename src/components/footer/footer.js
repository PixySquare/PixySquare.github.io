/** @jsxRuntime classic */
/** @jsx jsx */
import { jsx, Box, Text, Image ,Container } from 'theme-ui';
import logo from '../../assets/images/logo.png';
import { Link } from 'components/link';
import Widget from './widget';
import { menuItems, touchItems } from './footer.data';
    ...
    <Box as="footer" sx={styles.footer}>
      <Container>
        <Box sx={styles.footerTopInner}>
          <Box sx={styles.about}>
            <Box sx={styles.logo}>
              <Image sx={styles.logo} src={logo} alt="logo" width="48" height="50" />
              
              {/* <Logo /> */}
            </Box>
            <Box sx={styles.terms}>
              <Link path="#!">Terms of use</Link>
              {/* {touchItems.map(({ link, id, item }) => (
                <Link href={link} key={id}>{item}</Link>
              ))} */}
              <Text as="span">|</Text>
              <Link path="#!">Privacy</Link>
            </Box>
            <Text as="p" sx={styles.copyright}>
              Copyright by {new Date().getFullYear()} PixySquare Inc
            </Text>
          </Box>
